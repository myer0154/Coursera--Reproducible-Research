...
``` {r} 
hp1 <- ggplot(head(injuries, 5), aes(x=reorder(EVTYPE, -INJURIES), y=INJURIES))
hp1 <- hp1 + geom_bar(stat = "identity")
hp2 <- ggplot(head(fatalities, 5), aes(x=reorder(EVTYPE, -FATALITIES), y=FATALITIES))
hp2 <- hp2 + geom_bar(stat = "identity")
grid.arrange(hp1, hp2, ncol=2)
```